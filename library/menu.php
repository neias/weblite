...
class menu {
    ...
    public function render(array $attrs = NULL, $current = NULL, array $items = NULL) {
        ...
        $menu = '<ul'.($i == 1 ? self::attributes($attrs) : NULL). ($items !== NULL ? ' class="dropdown-menu" role="menu"' : '') .'>';
        ...
        foreach ($items as $key => $item) {
            ...
            $has_children ? $class[] = 'parent dropdown' : NULL;
            ...
            $menu .= '<li'.$classes.'><a href="'.$item['url']. ($has_children ? ' class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"' : '' ) . '">'.$item['title'].'</a>';
